...
async def test_email_sync_returns_503_when_no_streams_marked(monkeypatch):
    ...
    fake_connections = [
        {"id": "conn-google-1", "provider": "google", "provider_email": "a@example.com"}
    ]
    monkeypatch.setattr(
        email_router,
...
@pytest.mark.asyncio
async def test_email_sync_returns_accepted_response_when_streams_marked(monkeypatch):
    from api.routers import email as email_router
    import lib.supabase_client as supabase_client_module
    from api.services.syncs import sync_dispatcher
    ...
    fake_connections = [
        {"id": "conn-google-1", "provider": "google", "provider_email": "a@example.com"},
        {"id": "conn-ms-1", "provider": "microsoft", "provider_email": "b@example.com"},
    ]
    monkeypatch.setattr(
        email_router,
        "get_authenticated_supabase_client",
